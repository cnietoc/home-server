"""Gestor de secretos desde OneDrive (descarga bajo demanda, caché en memoria)."""
...
class SecretsManager:
    """Carga secretos desde OneDrive con caché en memoria (TTL 5 minutos)."""
    ...
    def fetch_platform_secrets_as_dict(self) -> dict:
        """Descarga secrets/platform.yml desde OneDrive (cached)."""
    ...
    def fetch_stack_secrets_as_dict(self, stack_name: str) -> dict:
        """Descarga secrets/stacks/<stack>.yml desde OneDrive (cached). {} si no existe."""
        return self._get_cached(
    ...
    def _download_and_parse(self, remote_path: str, default: dict | None = None) -> dict:
        """Descarga YAML de OneDrive y lo parsea."""
    ...
    def _get_access_token(self) -> str:
        """Obtiene access_token usando refresh_token (cacheado hasta expiración)."""
        now = time.time()
        ...
        self._access_token_expiry = now + 3300  # Cache por 55 min
        return self._access_token

    def _download_from_onedrive(self, remote_path: str) -> str | None:
        """Descarga fichero desde OneDrive via Graph API. Retorna None si 404."""
        try:
