...
# OneDrive Manager - Gestión completa de OneDrive con rclone
# Instalación, configuración, diagnóstico y monitoreo de OneDrive
...
show_summary() {
    ...
    echo
    info "ℹ️ Configuración de logs: Solo se registran ERRORES para minimizar ruido"
}
...
repair_service() {
    ...
    # Recrear el servicio con la configuración corregida
    setup_systemd_service "$HOME/OneDrive"
...
show_rclone_logs() {
    ...
    echo
    info "ℹ️ Configurado para mostrar solo ERRORES"
    echo
    ...
    info "   • CRITICAL: Errores críticos"
...
debug_rclone() {
    ...
    if [[ -f "$log_file" ]]; then
        ...
        if [[ $error_count -eq 0 && $timeout_count -eq 0 ]]; then
            log "✅ No hay errores recientes en el log"
        fi
    else
    ...
    echo
    echo "  # Montar manualmente con más información:"
    echo "  rclone mount onedrive: ~/OneDrive --log-level INFO --log-file ~/rclone-debug.log"
}
...
show_help() {
    ...
    echo "  logs              - Ver logs de rclone en tiempo real (solo errores)"
    ...
    echo "  ~/.local/share/rclone/onedrive-mount.log  - Logs de errores de rclone"
    echo "  ~/.config/rclone/rclone.conf              - Configuración de rclone"
    echo "  ~/OneDrive/                               - Directorio de montaje"
    echo
    echo "PROBLEMAS COMUNES:"
    echo "  • rclone se queda pillado  → $0 logs (para ver errores)"
    ...
    echo "  • Solo se registran ERRORES para minimizar ruido"
    echo "  • Los logs normales se ven en journalctl del servicio"
    echo "  • Para más detalle temporal: montar manualmente con --log-level INFO"
}
